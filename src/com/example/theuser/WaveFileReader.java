...
//            System.out.println(String.valueOf(this.len));  
//            System.out.println(System.currentTimeMillis());
            
            this.data = new int[this.numchannels][this.len];
            this.save = new float[this.numchannels][this.len];
            ...
            for(int i=0; i<this.len; ++i){  
                for(int n=0; n<this.numchannels; ++n){  
                    if(this.bitspersample == 8){  
                        ...
                        if(this.data[n][i]<0){
                        	this.save[n][i] = this.data[n][i]/(float)32768;
                        }
                        else{
                        	this.save[n][i] = this.data[n][i]/(float)32767;
                        }
...
//            for(int i=0; i<this.len; ++i)
//            {
//            	System.out.println(this.save[1][i]);
//            }
    ...
    private int readInt(){  
        byte[] buf = new byte[2];//读入两个字节，每个字节是8位  
        int res = 0;  
        try {  
            if(bis.read(buf)!=2)  
                throw new IOException("no more data!!!");  
            res = (buf[0]&0x000000FF) | (((int)buf[1])<<8);//int型整数是32位数，将两个8位字节表示的数转换成32位int型  
        } catch (IOException e) {  
            e.printStackTrace();  
        }  
        return res;  
    ...
    private long readLong(){  
        ...
        try {  
            long[] l = new long[4];  
            for(int i=0; i<4; ++i){  
                l[i] = bis.read();  //read()方法每次读入一个字节，8位数据
                if(l[i]==-1){  
                    throw new IOException("no more data!!!");  
                }  
            }  
